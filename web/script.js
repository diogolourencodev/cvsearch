...
function dataFormat(dataString) {
    ...
    // Remove caracteres inválidos do final (como o ']' no seu exemplo)
...
function filterCriticalVulnerabilities(cveData) {
    const RISK_THRESHOLDS = {
        CVSS_CRITICAL: 9.0,      // CVSS ≥ 9.0 = Critical
        CVSS_HIGH: 7.0,          // CVSS ≥ 7.0 = High
        EPSS_HIGH: 0.75,         // EPSS ≥ 75% = High exploitation
        EPSS_MEDIUM: 0.5         // EPSS ≥ 50% = Medium
    };
...
function getRiskColor(riskLevel) {
    const colors = {
        CRITICAL_KEV: '#ff0000',   // Red
        CRITICAL: '#ff4500',       // Strong orange
        HIGH: '#ff8c00',           // Orange
...
function displayReferences(references) {
    ...
    let html = '<div class="references-container"><h2>References:</h2><ul>';    
            ...
            if (isCVEID(searchTerm)) {
                ...
                resultsContainer.innerHTML = `
                    ...
                    <ul class="cve-info">
                        ...
                        <li><strong>Known Exploited:</strong> ${filtered.metrics.isKEV ? 'Yes 🔥' : 'No'}</li>
